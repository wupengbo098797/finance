<?php
	...
	// 2选择数据库
	mysql_select_db('myzhaolian',$conn);						//选择数据库 select = 查找 在1809下查找$conn
	
	// 3执行sql语句
	$sqlstr="select * from zhaolian where username='$username'";	//在from学生表单里面查找id
	...
	if(mysql_num_rows($result)>0){							//如果找到则>0 
		...
		$sqlstr1 = "insert into myzhaolian(username,userpass) values('".$username."','".$userpass."')";
